...
public class IngredientInput extends Ingredient {
    ...
    public Ingredient getInput() {
        if (!input.getInputs().isEmpty() && input.getInputs().get(0).hasTag()) {
            if (input.getInputs().size() == 1) {
                ...
                input.getInputs().forEach(stack -> itemList.add(stack.getItem()));
                return PartialNBTIngredient.of(input.getInputs().get(0).getTag(), itemList.toArray(new Item[0]));
            }
        } else {
    ...
    public IntList getStackingIds() {
        if (this.list == null) {
            final ItemStack[] itemsArr = this.getItems();
            this.list = new IntArrayList(itemsArr.length);
            for (final ItemStack itemstack : itemsArr) {
